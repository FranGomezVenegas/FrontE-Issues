...
import '../03config/proc-deploy-sample-sampling-settings';
import {windowSettings} from '../03config/proc-deploy-sample-sampling-settings'; //'../03config/config-process.js';
...
class procDeploySampleSampling extends FieldsMethods(FunctionsEnvMonitSamples(FrontendEndpointsEnvMonitSamples(connect(store)(PolymerElement)))) {
    static get properties() {
        return {
            tableDefinition: {type: Object, value:windowSettings},
            samplesInSampling: {type: Array, notify:true},
    ...
    stateChanged(state) {
        ...
        if (state.procDeploy!=null){
            this.samplesInSampling= state.procDeploy.samplesInSampling;}
    }        
    static get template() {
        return html`            
            <style include="proc-deploy-sample-sampling-style"></style> 
            ...
            <table-with-buttons id="procdeploy-samplefq" modulearea="env-monit-sample" selected_language="{{selectedLanguage}}"  
                element_definition="[[tableDefinition]]" table_data="{{samplesInSampling}}" 
    ...
    loadSamplingTable(){
        ...
        this.getAllSamplesStageSampling(this.tableDefinition,this.samplesStagesReduxVariables.SAMPLES_IN_SAMPLING);
    } 
    ready() {
        super.ready();
        this.loadSamplingTable();
...
customElements.define('proc-deploy-sample-sampling', procDeploySampleSampling);