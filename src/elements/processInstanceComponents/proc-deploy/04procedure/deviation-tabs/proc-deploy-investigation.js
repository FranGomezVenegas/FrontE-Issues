...
import '../../03config/ResultDeviation/proc-deploy-investigation-settings';
import {windowDefinition} from '../../03config/ResultDeviation/proc-deploy-investigation-settings'; //'../03config/config-process.js';

class procDeployInvestigation extends FieldsMethods(FunctionsEnvMonit(FrontendEndpointsEnvMonitForInvestigation(connect(store)(PolymerElement)))) {
    ...
    static get template() {
        return html`            
            <style include="proc-deploy-investigation-style"></style> 
    ...
    stateChanged(state) {
        ...
        if (state.procDeploy!=null){
            this.openInvestigations= state.procDeploy.openInvestigations;
        }
    }        
    ready() {
        super.ready();
...
customElements.define('proc-deploy-investigation', procDeployInvestigation);