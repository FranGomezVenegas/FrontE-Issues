...
class procDeploySampleReviewsampletestinggroupfq extends FieldsMethods(FunctionsEnvMonitSamples(FrontendEndpointsEnvMonitSamples(connect(store)(PolymerElement)))) {
    ...
    static get template() {
        return html`            
            ...
            <table-with-buttons id="procdeploy-samplefq" modulearea="env-monit-sample" selected_language="{{selectedLanguage}}"  
                element_definition="[[tableDefinition.elementdefinition]]" table_data="{{samplePendingRevisionTheTestingGroupFQ}}" 
    ...
    loadSamplingTable(){
        ...
        this.getAllSamplesStageSampling(this.tableDefinition.apicall, this.samplesStagesReduxVariables.SAMPLES_PENDINGREVISION_TESTINGGROUPFQ);
    } 
    ready() {
        super.ready();
        this.loadSamplingTable();
