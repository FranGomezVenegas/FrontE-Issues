...
class procDeploySampleFq extends FieldsMethods(FunctionsEnvMonitSamples(FrontendEndpointsEnvMonitSamples(connect(store)(PolymerElement)))) {
    ...
    static get template() {
        return html`            
            ...
            <table-with-buttons id="procdeploy-samplefq" modulearea="env-monit-sample" selected_language="{{selectedLanguage}}"  
                element_definition="[[tableDefinition.elementdefinition]]" table_data="{{samplesInProgressFQ}}" 
    ...
    loadSamplingTable(){
        ...
        this.getAllSamplesStageSampling(this.tableDefinition.apicall,this.samplesStagesReduxVariables.SAMPLES_IN_PROGRESS_FQ);
    } 
    ready() {
        super.ready();
        this.loadSamplingTable();
