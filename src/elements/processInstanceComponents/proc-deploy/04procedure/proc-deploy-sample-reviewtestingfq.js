...
class procDeploySampleReviewtestingfq extends FieldsMethods(FunctionsEnvMonitSamples(FrontendEndpointsEnvMonitSamples(connect(store)(PolymerElement)))) {
    ...
    static get template() {
        return html`            
            ...
            <table-with-buttons id="procdeploy-samplefq" modulearea="env-monit-sample" selected_language="{{selectedLanguage}}"  
                element_definition="[[tableDefinition.elementdefinition]]" table_data="{{samplesAnalysisPendingRevisionFQ}}" 
    ...
    loadSamplingTable(){
        ...
        this.getAllSamplesStageSampling(this.tableDefinition.apicall, this.samplesStagesReduxVariables.SAMPLEANALYSIS_PENDINGREVISION_FQ);
    } 
    ready() {
        super.ready();
        this.loadSamplingTable();
