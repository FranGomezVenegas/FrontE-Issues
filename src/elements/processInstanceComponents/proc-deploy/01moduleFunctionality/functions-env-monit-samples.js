...
sampleLogButtonClicked(e){      
    ...
    for (iField = 0; iField < this.selectedSamplingPoint.length; iField++) {        
        ...
        if (this.selectedSamplingPoint[iField].name!='order_number' && 
          this.selectedSamplingPoint[iField].name!='description_es' && 
          this.selectedSamplingPoint[iField].name!='description_en' && 
          String(this.selectedSamplingPoint[iField].value).length>0){
            ...
            if (fieldType!=undefined){ //this.selectedSamplingPoint[iField].type;}
                switch (fieldType) {
                    case 'date':
                    case 'Date':
                    ...
                    case 'list':
                    case 'List':
                        var fldValue=this.selectedSamplingPoint[iField].value;
                        if (fldValue.includes('-')){
                            fldValue=fldValue.substring(0, fldValue.indexOf('-'));
                            alert("*"+this.selectedSamplingPoint[iField].value+ "* >>> *"+fldValue+"*");
                        }
                        fieldsStringNames=fieldsStringNames+this.selectedSamplingPoint[iField].name;
                        fieldsStringValues=fieldsStringValues+fldValue+'*String';
                        break;
                    default:
                        this.dispatchEvent(new CustomEvent('toast-error', {
                            bubbles: true,
                            composed: true,
                            detail: 'The field type '+this.selectedSamplingPoint[iField].dbType+' for the field '+this.selectedSamplingPoint[iField].name+' is not implemented yet in onButonClicked then this field is discarded'
                            }));                        
                        break;
                    }
                }
